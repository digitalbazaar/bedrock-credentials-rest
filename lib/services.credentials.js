...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
    ...
    rest.makeResourceHandler({
      ...
      */
      get: function(req, res, callback) {
        ...
        if(!req.user.identity && req.query.id) {
          // FIXME: return a credential marked as public
          // if no public crendentials, respond with 404
          var publicQuery = {
            'credential.id': req.query.id,
            'credential.sysPublic': '*'
          };
          return store.getAll(null, publicQuery, function(err, records) {
            if(records.length === 0) {
        ...
        if(req.query.id) {
          return async.waterfall([
            function(callback) {
              store.get(req.user.identity, req.query.id, callback);
            },
            function(credential, meta, callback) {
              if(!_checkCredentialParties(req.user.identity, credential)) {
                return callback(new BedrockError(
                  'Credential not found.', 'NotFound', {
                    public: true,
                    httpStatusCode: 404
              ...
              callback(null, credential, meta);
            },
            function(credential, meta, callback) {
              _credentialToFormat(
                credential, meta, req.query.format, callback);
            }
          ], function(err, result) {
            if(err) {
              return callback(err);
  ...
  app.post(basePath + '/:credential',
    ...
    function(req, res, next) {
      // get ID from URL
      var credentialId =
        bedrock.config.server.baseUri + basePath + '/' + req.params.credential;
      // check id matches
      ...
      // TODO: include in the update that the sysState is unclaimed, otherwise
      // do not do the claim again (important for provenance/logging, etc)
      var updateQuery = {'credential.id': credentialId};
      var now = Date.now();
      var updateParams = {
        $set: {
  ...
  function _checkCredentialParties(identity, credential) {
    var issuer = credential.issuer;
    var holder = credential.claim.id;
    if(identity.id === issuer || identity.id === holder) {
      return true;
    }
    return false;
  }
