...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  //app.post(basePath + '/credentials',
  //  ensureAuthenticated,
  //  validate('services.credential-rest.postCredentials'),
  //  function(req, res, next) {
  //    // ...
  //  });

  // FIXME: add CORS and access control support
  app.get(basePath + '/credentials',
    ensureAuthenticated,
    rest.makeResourceHandler({
      /* FIXME
      validate: {
        query: 'services.credential-rest.getCredentialsQuery'
      },
      */
  ...
  docs.annotate.get(basePath + '/credentials', {
  ...
  /*
  app.post(basePath + '/credentials/:credential',
    ensureAuthenticated,
    validate('services.credential.postCredential'),
    function(req, res, next) {
      // get ID from URL
  ...
  docs.annotate.post(basePath + '/credentials/:credential', {
    description: 'Modify an existing credential.',
    securedBy: ['cookie', 'hs1'],
    schema: 'services.credentials.postCredential',
    responses: {
  ...
  function _getCredentialRecord(req, res, callback) {
    ...
    async.auto({
      ...
      issuer: ['record', function(callback, results) {
        if(actor) {
          return ....checkCredentialIssuer(
            actor, results.record.credential, callback);
        }
        callback(null, false);
      }],
      recipient: ['record', function(callback, results) {
        if(actor) {
          return ....checkCredentialRecipient(
            actor, results.record.credential, callback);
        }
        callback(null, false);
      }],
      filteredCredential: ['record', 'issuer', 'recipient',
        function(callback, results) {
        var credential = results.record.credential;
        if(results.issuer || results.recipient) {
  ...
  */
  ...
  app.options(basePath + '/credentials/:credential', cors());
  app.get(basePath + '/credentials/:credential',
    cors(), optionallyAuthenticated, rest.makeResourceHandler({
      validate: {
        query: 'services.credential.getCredentialQuery'
      },
      get: function(req, res, callback) {
  ...
  docs.annotate.get(basePath + '/credentials/:credential', {
  ...
  /*
  app.delete(basePath + '/credentials/:credential',
    ensureAuthenticated,
    function(req, res, next) {
